...
let regex = /^([^_])(.+)\.scss$/;
let cssdir = fs.readdirSync('./src/assets/scss');
let css = cssdir.filter(c => c.match(regex));

for (let i in css) {
	if (css.hasOwnProperty(i)) {
		let cssfilename = './src/public/css/' + css[i].replace('scss', 'css');
		let cssmapfilename = './src/public/css/' + css[i].replace('scss', 'css.map');
		let tmp = {
			file: './src/assets/scss/' + css[i],
			outFile: cssfilename,
