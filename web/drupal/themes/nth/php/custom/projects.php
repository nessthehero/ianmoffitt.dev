...
class ProjectsQuery {
	...
	protected $machine = array('project');
	...
	protected function filter() {
		...
		$cache_key = "project-" . date("Y-m-d-h", time()) . $cache_tax;
...
function build_project_search($node, &$variables, &$_q) {
	...
	$project = new ProjectsQuery($taxonomy);
	$project->filterByTaxonomy('field_lab_tags', $q_center);
	$project->filterByTaxonomy('field_center_tags', $q_lab);

	$project->filterByDepartment($q_center);
	$project->filterByResearch($q_lab);

	$q_debug = nvl($_q, 'debug');

	$variables['project_query'] = $project;
	...
	$variables['project'] = $project->results($count, $variables['offset']);
	if (!empty($q_debug)) {
		$variables['project_count'] = (int) $q_debug; //$project->count(true);
		$variables['project_count_un'] = (int) $q_debug; //$project->count();
		$variables['page_count'] = (int) $q_debug / 10; //$project->pageCount();
	} else {
		$variables['project_count'] = $project->count(true);
		$variables['project_count_un'] = $project->count();
		$variables['page_count'] = $project->pageCount();
	}
